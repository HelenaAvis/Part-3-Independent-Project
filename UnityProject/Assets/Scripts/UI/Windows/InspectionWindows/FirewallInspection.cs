...
public class FirewallInspection : MonoBehaviour
{
    ...
    [SerializeField] private GameObject portListContent;
    [SerializeField] private GameObject portListButtonPrefab;
    ...
    public void Initialise()
    {
        ...
        foreach (FirewallPort port in target.firewallPorts)
        {
            portButton = Instantiate(portListButtonPrefab, portListContent.transform);
            portButton.name = port.portNumber + " Button";
            portButton.GetComponentInChildren<TextMeshProUGUI>().text = port.portNumber.ToString();
        }
